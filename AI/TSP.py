...
class MainWindow(QWidget):
    def __init__(self):
        ...
        self.delay_opt2 = 0.0
        self.init_ui()
        self.show()  # Use show() to display the main window

    def init_ui(self):
        self.setWindowTitle('TSP AI Project')
...
class CityMap(FigureCanvasQTAgg):
    def __init__(self, parent=None):
        ...
        self.fig = plt.figure()
        self.init_city_line = None  # Initialize init_city_line attribute
    ...
    def remove_city(self, event):
        ...
        if isinstance(thisline, mpl.lines.Line2D):
            # If 'thisline' is an individual line object
            thisline.remove()
        else:
            # If 'thisline' is a collection of lines
    ...
    def select_init_city(self, event):
        ...
        if mouseevent.button != 1:
            return
        thisline = event.artist
        if self.init_city_line is None:
            self.init_city_line = thisline
            thisline.remove()  # Remove the clicked city from the canvas
            if len(self.ax.lines) > 0:
                self.ax.lines[0].set_color('r')  # Change color of previous initial city line
            self.draw()
